...
class UpdateHealthInsurance extends StatefulWidget {
  ...
  @override
  State<UpdateHealthInsurance> createState() => _UpdateContactState();
}

class _UpdateContactState extends State<UpdateHealthInsurance> {
  final _formKey = GlobalKey<FormState>();
  bool _loading = false;
  late Map<String, dynamic> _healthInsurance = {};
  ...
  Future<void> _handleSave() async {
    ...
    try {
      List<String> healthInsurance = [];
      _healthInsurance.forEach((key, value) {
        healthInsurance.add(value);
      });
      final AppUser user = AuthService().currentUser!;
      await UsersService().updateUserHealthInsurance(
        userId: user.id,
  ...
  _addFiel() {
    _healthInsurance.putIfAbsent(_healthInsurance.length.toString(), () => "");
    setState(() {});
  }
  ...
  _removeFiel(String objectKey) async {
    ...
    final services = {..._healthInsurance};
    ...
    for (int i = 0; i < servicesList.length; i++) {
      _healthInsurance.putIfAbsent(i.toString(), () => servicesList[i]);
    }
  ...
  void initState() {
    ...
    if (services.isNotEmpty) {
      for (int i = 0; i < services.length; i++) {
        _healthInsurance.putIfAbsent(i.toString(), () => services[i]);
      }
    } else {
      _healthInsurance = {"0": ""};
    }
  ...
  Widget build(BuildContext context) {
    final traslation =
        Translations.of(context).translate('update_health_insurance');
    List<Widget> inputs() {
      ...
      _healthInsurance.forEach((key, value) {
        res.add(Stack(
          children: [
            if (!_loading)
              CustomInput(
                formData: _healthInsurance,
                objectKey: key,
                onChanged: (val) => _healthInsurance[key] = val,
                label: traslation["plan"].replaceAll(
                  "{amount}",
                  "${int.parse(key) + 1}",
                ),
              )
