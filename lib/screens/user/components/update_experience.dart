...
class UpdateExperience extends StatefulWidget {
  ...
  @override
  State<UpdateExperience> createState() => _UpdateContactState();
}

class _UpdateContactState extends State<UpdateExperience> {
  final _formKey = GlobalKey<FormState>();
  bool _loading = false;
  late Map<String, dynamic> _expaeriences = {};
  ...
  Future<void> _handleSave() async {
    ...
    try {
      List<String> expaeriences = [];
      _expaeriences.forEach((key, value) {
        expaeriences.add(value);
      });
      final AppUser user = AuthService().currentUser!;
      await UsersService().updateUserExperiences(
        userId: user.id,
        experiences: expaeriences,
      );
  ...
  _addFiel() {
    _expaeriences.putIfAbsent(_expaeriences.length.toString(), () => "");
    setState(() {});
  }
  ...
  _removeFiel(String objectKey) async {
    ...
    for (int i = 0; i < servicesList.length; i++) {
      _expaeriences.putIfAbsent(i.toString(), () => servicesList[i]);
    }
  ...
  void initState() {
    ...
    if (services.isNotEmpty) {
      for (int i = 0; i < services.length; i++) {
        _expaeriences.putIfAbsent(i.toString(), () => services[i]);
      }
    } else {
      _expaeriences = {"0": ""};
    }
  ...
  Widget build(BuildContext context) {
    final traslation = Translations.of(context).translate('update_experience');
    List<Widget> inputs() {
      ...
      _expaeriences.forEach((key, value) {
        res.add(Stack(
          children: [
            if (!_loading)
              CustomInput(
                formData: _expaeriences,
                objectKey: key,
                onChanged: (val) => _expaeriences[key] = val,
                label: traslation["experience"]
